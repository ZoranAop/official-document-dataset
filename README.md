...
> **说明**：当前检索基于 SQLite + 关键词索引实现，属于多条件结构化检索。未来可扩展至向量检索和语义搜索。
...
- **Phase 1** (已完成): SQLite + 关键词索引
- **Phase 2** (规划中): SQLite FTS5 全文检索
- **Phase 3** (规划中): Embedding 向量化
- **Phase 4** (规划中): Hybrid 混合检索

### 功能扩展

1. **全文检索** - 集成Elasticsearch，支持复杂查询
2. **Web界面** - Vue.js前端展示
3. **定时任务** - 每日增量更新
4. **MCP服务** - 提供AI Agent工具接口
5. **证据链追溯** - 结构模式 → 源文档 → 原文链接
