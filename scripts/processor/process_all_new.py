        ...
        for i, article in enumerate(articles):
            ...
            try:
                processed_doc = {
                    ...
                    'source': '12371.cn' if '12371' in category else 'jhsjk.people.cn',
                    'source_url': article.get('url', ''),
                    'category': category,
                    'document_type': self._infer_type(article.get('title', '')),
                    'document_subtype': category.replace('12371_', '').replace('jhsjk_', ''),
                    'domains': [],
                    'event': '',
                    'location': '',
                    'subjects': [],
...
def main():
    ...
    parser = argparse.ArgumentParser(description='处理所有新增数据')
    ...
    if wk_file.exists():
        ...
        processor.process_articles(articles, '12371_wk', Path('data/structured/cnorg_wk_documents.jsonl'))
    ...
    if ls_file.exists():
        ...
        processor.process_articles(articles, '12371_ls', Path('data/structured/cnorg_ls_documents.jsonl'))
    
    # 处理时政热点 (zs)
    zs_file = Path(args.data_dir) / 'index' / 'special' / 'zs' / 'zs_articles.json'
    if zs_file.exists():
        with open(zs_file, 'r', encoding='utf-8') as f:
            data = json.load(f)
            articles = data.get('articles', [])
        logger.info(f"Processing zs: {len(articles)} articles")
        processor.process_articles(articles, '12371_special_zs', Path('data/structured/cnorg_zs_documents.jsonl'))
    
    # 处理习近平新时代中国特色社会主义思想 (hxnr)
    hxnr_file = Path(args.data_dir) / 'index' / 'special' / 'hxnr' / 'hxnr_articles.json'
    if hxnr_file.exists():
        with open(hxnr_file, 'r', encoding='utf-8') as f:
            data = json.load(f)
            articles = data.get('articles', [])
        logger.info(f"Processing hxnr: {len(articles)} articles")
        processor.process_articles(articles, '12371_special_hxnr', Path('data/structured/cnorg_hxnr_documents.jsonl'))
    
    # 处理党建领域 (party)
    party_file = Path(args.data_dir) / 'index' / 'jhsjk_party' / 'party_articles.json'
    if party_file.exists():
        with open(party_file, 'r', encoding='utf-8') as f:
            data = json.load(f)
            articles = data.get('articles', [])
        logger.info(f"Processing party: {len(articles)} articles")
        processor.process_articles(articles, 'jhsjk_party', Path('data/structured/jhsjk_party_documents.jsonl'))
